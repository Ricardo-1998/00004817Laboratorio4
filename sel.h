void createLocalA(Matrix &A,mesh m){
    float t = m.getParameter(T_NUEVA);
    A.at(0).at(0) += -t/8;
    A.at(0).at(1) += t/8;
    A.at(1).at(0) += -t/8;
    A.at(1).at(1) += t/8;
...
void createLocalB(Matrix &B,mesh m){
    ...
    float k = m.getParameter(K_CONSTANTE);
    B.at(0).at(0) += k/l;      B.at(0).at(1) += -k/l;
    B.at(1).at(0) += -k/l;     B.at(1).at(1) += k/l;
}

void createLocalC(Matrix &C,mesh m){
    float e = m.getParameter(EPSILONCAT);
    C.at(0).at(0) += -e/3;    C.at(0).at(1) += e/3;
    C.at(1).at(0) += -e/3;    C.at(1).at(1) += e/3;
}

void createLocalD(Matrix &D,mesh m){
    float v = m.getParameter(V_CONSTANTE);
    float length = m.getParameter(ELEMENT_LENGTH);
    D.at(0).at(0) += v/length;  D.at(0).at(1) += -v/length;
    D.at(1).at(0) += -v/length;  D.at(1).at(1) += v/length;
}
...
void createLocalF(Matrix &D,mesh m){
    float s = m.getParameter(SIGMA);
    D.at(0).at(0) += -s/2;  D.at(0).at(1) += s/2;
    D.at(1).at(0) += -s/2;  D.at(1).at(1) += s/2;
}
...
Vector createLocalb(int element,mesh &m){
    ...
    float f = m.getParameter(PHI);
    float mew = m.getParameter(MEW);
    float length = m.getParameter(ELEMENT_LENGTH);
    b.push_back(f*length/2);
    b.push_back(f*length/2);
    b.push_back(mew*length/2);
    b.push_back(mew*length/2);
